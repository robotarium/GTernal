...
TOF_SENSOR_ANGLE = [-np.pi, np.pi/2, np.pi/4, 0, -np.pi/4, -np.pi/2, -np.pi]

def create_single_integrator_barrier_certificate(barrier_gain=100, safety_radius=0.17, magnitude_limit=0.2, safety_margin = 0.05):
    ...
    def f(dxi, ToF_sensor_values):
        ...
        assert isinstance(ToF_sensor_values, np.ndarray), "In the function created by the create_single_integrator_barrier_certificate function, the ToF sensor values (ToF_sensor_values) must be a numpy array. Recieved type %r." % type(ToF_sensor_values).__name__
        ...
        assert ToF_sensor_values.shape[0] == 7, "In the function created by the create_single_integrator_barrier_certificate function, the dimension of the ToF sensor values (ToF_sensor_values) must be 7 ([ToF1;ToF2;ToF3;ToF4:ToF5,ToF6,ToF7]). Recieved dimension %r." % ToF_sensor_values.shape
        ...
        for i in range(7):
        # i = 3
            error = ToF_sensor_values[i]
            h = error*error - np.power(safety_radius, 2)
            # h_offset = np.power(safety_radius, 2) - safety_margin*safety_margin

            A[i, :] = 2*error*np.array([np.cos(TOF_SENSOR_ANGLE[i]), np.sin(TOF_SENSOR_ANGLE[i])])
            b[i] = barrier_gain*np.power(h, 3)
            # b[i] = barrier_gain*np.power(h, 3) - barrier_gain*np.power(h_offset, 3)
...
def create_single_integrator_barrier_certificate2(barrier_gain=100, unsafe_barrier_gain=1e6, safety_radius=0.05, magnitude_limit=0.2):
    """Creates a barrier certificate for a single-integrator system.  This function
    ...
    def f(dxi, ToF_sensor_values):
        ...
        for i in range(7):
            ...
            if h >= 0:
                ...
                b[i] = unsafe_barrier_gain*np.power(h, 3)
